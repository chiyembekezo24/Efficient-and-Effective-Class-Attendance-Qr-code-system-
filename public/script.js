...
function initNavigation() {
    ...
    navButtons.forEach(button => {
        button.addEventListener('click', () => {
            ...
            // Stop auto-refresh if leaving dashboard
...
async function loadRecentActivity() {
    try {
                ...
                <div class="activity-item">
                    <div class="activity-content">
                        <div class="activity-main">
                            <i class="fas fa-user-check" style="color: #48bb78; margin-right: 8px;"></i>
                            <strong>${student ? student.name : 'Unknown Student'}</strong> 
                            <span style="color: #718096;">marked attendance for</span>
                        ...
                        <div class="activity-details">
                            <span style="color: #718096; font-size: 0.9rem;">
                                ...
                                ${record.location && record.location.address ? `• 📍 ${record.location.address}` : ''}
                            </span>
                        </div>
                    </div>
        ...
        const activityList = document.getElementById('recentActivity');
        activityList.innerHTML = '<p class="no-data">Failed to load recent activity</p>';
    }
}

// Auto-refresh functions for recent activity
...
function displayReportDetail(report) {
    ...
    const detailHTML = `
        ...
        <div class="student-list">
            <h4><i class="fas fa-check-circle" style="color: #48bb78; margin-right: 8px;"></i>Present Students (${report.presentStudents.length})</h4>
            ${report.presentStudents.length > 0 ? report.presentStudents.map(student => `
                <div class="student-item">
                    <div class="student-info">
                        <div class="student-name">${student.name}</div>
                        <div class="student-id">ID: ${student.studentId}</div>
                        ${student.email ? `<div class="student-email">Email: ${student.email}</div>` : ''}
                        ${student.location && student.location.address ? `<div class="student-location"><i class="fas fa-map-marker-alt"></i> ${student.location.address}</div>` : ''}
                    </div>
                    <div class="student-time">${formatTime(student.timestamp)}</div>
                </div>
            `).join('') : '<p class="no-data">No students marked present</p>'}
        </div>
        
        <div class="student-list">
            <h4><i class="fas fa-times-circle" style="color: #f56565; margin-right: 8px;"></i>Absent Students (${report.absentStudents.length})</h4>
            ${report.absentStudents.length > 0 ? report.absentStudents.map(student => `
                <div class="student-item">
                    <div class="student-info">
                        <div class="student-name">${student.name}</div>
                        <div class="student-id">ID: ${student.studentId}</div>
                    ...
                    <div class="student-status">Absent</div>
                </div>
            `).join('') : '<p class="no-data">All enrolled students are present</p>'}
        </div>
    `;
